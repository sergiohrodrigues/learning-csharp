## Olá, esse é um repositório dedicado aos meus estudos na linguagem de programação C#.

### Todos os dias separo entre 1 e 2 horas do meu dia para estudar o curso do Nélio Alves.

### Obs: recomendo muito o curso dele, apesar de ser um pouco antigo ele explica de uma maneira bem fácil desde os primeiros contatos com a linguagem, e tem vários exercicios também para fixação do conteudo..

#### Curso: C# COMPLETO Programação Orientada a Objetos + Projetos
